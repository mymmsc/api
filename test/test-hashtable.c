...
int main(int argc, char *argv[])
{
    ...
    api_hashtable_t ht;
    api_hashtable_init(&ht, HT_KEY_CONST | HT_VALUE_CONST, 0.05);

    char *s1 = (char*)"teststring 1";
    char *s2 = (char*)"teststring 2";
    char *s3 = (char*)"teststring 3";

    api_hashtable_insert(&ht, s1, strlen(s1)+1, s2, strlen(s2)+1);

    int contains = api_hashtable_contains(&ht, s1, strlen(s1)+1);
    test(contains, "Checking for key \"%s\"", s1);

    size_t value_size;
    char *got = api_hashtable_get(&ht, s1, strlen(s1)+1, &value_size);
    ...
    api_hashtable_insert(&ht, s1, strlen(s1)+1, s3, strlen(s3)+1);

    unsigned int num_keys;
    void **keys;

    keys = api_hashtable_keys(&ht, &num_keys);
    test(num_keys == 1, "HashTable has %d keys", num_keys);
    test(keys != NULL, "Keys is not null");
    if(keys)
      free(keys);
    got = api_hashtable_get(&ht, s1, strlen(s1)+1, &value_size);
    ...
    api_hashtable_remove(&ht, s1, strlen(s1)+1);

    contains = api_hashtable_contains(&ht, s1, strlen(s1)+1);
    ...
    for(i = 0; i < key_count; i++)
    {
        api_hashtable_insert(&ht, &(many_keys[i]), sizeof(many_keys[i]), &(many_values[i]), sizeof(many_values[i]));
    }
    ...
    for(i = 0; i < key_count; i++)
    {
        if(api_hashtable_contains(&ht, &(many_keys[i]), sizeof(many_keys[i])))
        {
            size_t value_size;
            int value;

            value = *(int*)api_hashtable_get(&ht, &(many_keys[i]), sizeof(many_keys[i]), &value_size);
    ...
    api_hashtable_clear(&ht);
    api_hashtable_resize(&ht, 4194304);
    t1 = snap_time();
    ...
    for(i = 0; i < key_count; i++)
    {
        api_hashtable_remove(&ht, &(many_keys[i]), sizeof(many_keys[i]));
    }
    test(api_hashtable_size(&ht) == 0, "%d keys remaining", api_hashtable_size(&ht));
    api_hashtable_destroy(&ht);
    free(many_keys);
    free(many_values);
