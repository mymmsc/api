/**
 * @(#)PduFactory.java	6.3.9 09/10/02
 *
...
import org.mymmsc.w3c.gsm.pdu.wappush.WapSiPdu;
...
public class PduFactory {
	...
	// used to determine what PduContext to use based on the first octet
	// this is the only way to instantiate a PduContext object
	public static PduContext createPdu(int firstOctet) {
		PduContext pduContext = null;
		int messageType = getFirstOctetField(firstOctet, PduUtils.TP_MTI_MASK);
		switch (messageType) {
		case PduUtils.TP_MTI_SMS_DELIVER:
			pduContext = new SmsDeliveryPdu();
			break;
		case PduUtils.TP_MTI_SMS_STATUS_REPORT:
			pduContext = new SmsStatusReportPdu();
			break;
		case PduUtils.TP_MTI_SMS_SUBMIT:
			pduContext = new SmsSubmitPdu();
			break;
		default:
			throw new RuntimeException("Invalid TP-MTI value: " + messageType);
		}
		...
		pduContext.setFirstOctet(firstOctet);
		return pduContext;
	}
}
