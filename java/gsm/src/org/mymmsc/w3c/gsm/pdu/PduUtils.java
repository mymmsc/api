...
public class PduUtils {
	...
	// ==================================================
	private static Integer getTpField(PduContext pduContext, String fieldName) {
		try {
			Method m = pduContext.getClass().getMethod("get" + fieldName);
			return (Integer) m.invoke(pduContext);
		} catch (Exception e) {
		}
		return null;
	}

	private static Boolean hasTpField(PduContext pduContext, String fieldName) {
		try {
			Method m = pduContext.getClass().getMethod("has" + fieldName);
			return (Boolean) m.invoke(pduContext);
		} catch (Exception e) {
		}
		return null;
	}

	public static String decodeFirstOctet(PduContext pduContext) {
		// Use reflection to check for method signatures hasTpXXX() and
		// getTpXXX()
		// that are specific to a particular type in actual object
		StringBuffer sb = new StringBuffer();
		sb.append("First Octet: "
				+ PduUtils.byteToPdu(pduContext.getFirstOctet()));
		sb.append(" [");
		// TP-MTI
		...
		// TP-MMS
		if (hasTpField(pduContext, "TpMms") != null) {
			if (hasTpField(pduContext, "TpMms")) {
		...
		// TP-RD
		if (hasTpField(pduContext, "TpRd") != null) {
			if (hasTpField(pduContext, "TpRd")) {
		...
		// TP-VPF
		if ((hasTpField(pduContext, "TpVpf") != null)
				&& (hasTpField(pduContext, "TpVpf") != false)) {
			switch (getTpField(pduContext, "TpVpf")) {
		...
		// TP-SRI
		if (hasTpField(pduContext, "TpSri") != null) {
			if (hasTpField(pduContext, "TpSri")) {
		...
		// TP-SRR
		if (hasTpField(pduContext, "TpSrr") != null) {
			if (hasTpField(pduContext, "TpSrr")) {
	...
	public static String decodeDataCodingScheme(PduContext pduContext) {
		StringBuffer sb = new StringBuffer();
		switch (PduUtils.extractDcsEncoding(pduContext.getDataCodingScheme())) {
		...
		if ((pduContext.getDataCodingScheme() & ~PduUtils.DCS_CODING_GROUP_GENERAL) == 0) {
			switch (PduUtils.extractDcsClass(pduContext.getDataCodingScheme())) {
	...
	// from Java String to PduContext
	...
	public static byte[] stringToUnencodedSeptets(String s) {
		...
		for (i = 0; i < s.length(); i++) {
			...
			{
				baos.write((byte) Integer.parseInt(extBytes[index].substring(0,
						2), 16));
				k++;
				baos.write((byte) Integer.parseInt(extBytes[index].substring(2,
						4), 16));
				k++;
			} else
