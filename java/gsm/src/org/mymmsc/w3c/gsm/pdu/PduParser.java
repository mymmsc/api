...
public class PduParser {
	...
	private String readAddress(int addressLength, int addressType) {
		...
		if (addressLength > 0) {
			...
			case PduUtils.ADDRESS_TYPE_ALPHANUMERIC:
				...
				return stripAddressFieldPadding(addressLength, PduUtils
						.decode7bitEncoding(addressData));
			default:
	...
	public PduContext parsePdu(String rawPdu) {
		...
		PduContext pduContext = parseStart();
		pduContext.setRawPdu(rawPdu);
		// parse depending on the pdu type
		switch (pduContext.getTpMti()) {
		case PduUtils.TP_MTI_SMS_DELIVER:
			parseSmsDeliverMessage((SmsDeliveryPdu) pduContext);
			break;
		case PduUtils.TP_MTI_SMS_SUBMIT:
			parseSmsSubmitMessage((SmsSubmitPdu) pduContext);
			break;
		case PduUtils.TP_MTI_SMS_STATUS_REPORT:
			parseSmsStatusReportMessage((SmsStatusReportPdu) pduContext);
			break;
		}
		return pduContext;
	}

	private PduContext parseStart() {
		// SMSC info
		// length
		// address type
		// smsc data
		int addressLength = readByte();
		PduContext pduContext = null;
		if (addressLength > 0) {
			...
			pduContext = PduFactory.createPdu(firstOctet);
			// generic methods
			pduContext.setSmscAddressType(addressType);
			pduContext.setSmscAddress(smscAddress);
			pduContext.setSmscInfoLength(addressLength);
		} else {
			// first octet - determine how to parse and how to store
			int firstOctet = readByte();
			pduContext = PduFactory.createPdu(firstOctet);
	...
	private void parseUserData(PduContext pduContext) {
		// ud length
		...
		pduContext.setUDLength(udLength);
		// user data
		...
		if (pduContext.hasTpUdhi()) {
			...
			while (position < endUdh) {
				...
				pduContext.addInformationElement(ie);
	 ...
	 */
	private String stripAddressFieldPadding(int validSemiOctetNumber,
			String addressFieldString) {
