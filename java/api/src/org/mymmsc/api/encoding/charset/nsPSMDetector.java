...
package org.mymmsc.api.encoding.charset;
...
public abstract class nsPSMDetector {
	...
	protected void initVerifiers(int currVerSet) {
		...
		if (currVerifierSet == nsPSMDetector.TRADITIONAL_CHINESE) {
			...
			mVerifier = new nsVerifier[] { new nsUTF8Verifier(),
					new nsBIG5Verifier(), new nsISO2022CNVerifier(),
					new nsEUCTWVerifier(), new nsCP1252Verifier(),
					new nsUCS2BEVerifier(), new nsUCS2LEVerifier() };

			mStatisticsData = new nsEUCStatistics[] { null,
					new Big5Statistics(), null, new EUCTWStatistics(), null,
					null, null };
		}
		...
		// ==========================================================
			...
			mVerifier = new nsVerifier[] { new nsUTF8Verifier(),
					new nsEUCKRVerifier(), new nsISO2022KRVerifier(),
					new nsCP1252Verifier(), new nsUCS2BEVerifier(),
		...
		// ==========================================================
			...
			mVerifier = new nsVerifier[] { new nsUTF8Verifier(),
					new nsGB2312Verifier(), new nsGB18030Verifier(),
		...
		// ==========================================================
			...
			mVerifier = new nsVerifier[] { new nsUTF8Verifier(),
					new nsSJISVerifier(), new nsEUCJPVerifier(),
		...
		// ==========================================================
			...
			mStatisticsData = new nsEUCStatistics[] { null,
					new GB2312Statistics(), null, new Big5Statistics(), null,
		...
		// ==========================================================
			...
			mVerifier = new nsVerifier[] { new nsUTF8Verifier(),
					new nsSJISVerifier(), new nsEUCJPVerifier(),
					new nsISO2022JPVerifier(), new nsEUCKRVerifier(),
					new nsISO2022KRVerifier(), new nsBIG5Verifier(),
					new nsEUCTWVerifier(), new nsGB2312Verifier(),
					new nsGB18030Verifier(), new nsISO2022CNVerifier(),
					new nsHZVerifier(), new nsCP1252Verifier(),
					new nsUCS2BEVerifier(), new nsUCS2LEVerifier() };

			mStatisticsData = new nsEUCStatistics[] { null, null,
					new EUCJPStatistics(), null, new EUCKRStatistics(), null,
					new Big5Statistics(), new EUCTWStatistics(),
	...
	public boolean HandleData(byte[] aBuf, int len) {
		...
		for (i = 0; i < len; i++) {
			...
			for (j = 0; j < mItems;) {
				st = nsVerifier.getNextState(mVerifier[mItemIdx[j]], b,
						mState[j]);
				// if (st != 0)
	...
	public void Sample(byte[] aBuf, int aLen, boolean aLastChance) {
		...
		if (mRunSampler) {
			...
			if (((aLastChance && mSampler.GetSomeData()) || mSampler
					.EnoughData()) && (eucNum == possibleCandidateNum)) {
				mSampler.CalFreq();
				...
				for (j = 0; j < mItems; j++) {
					if ((null != mStatisticsData[mItemIdx[j]])
							&& (!(mVerifier[mItemIdx[j]].charset())
									.equals("Big5"))) {
						float score = mSampler
								.GetScore(mStatisticsData[mItemIdx[j]]
										.mFirstByteFreq(),
										mStatisticsData[mItemIdx[j]]
												.mFirstByteWeight(),
										mStatisticsData[mItemIdx[j]]
												.mSecondByteFreq(),
										mStatisticsData[mItemIdx[j]]
