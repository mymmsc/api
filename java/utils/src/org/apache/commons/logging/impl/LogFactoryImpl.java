 ...
 * @version $Revision: 581090 $ $Date: 2007-10-02 00:01:06 +0200 (ti, 02 okt
 *          2007) $
 */
...
public class LogFactoryImpl extends LogFactory {
	 ...
	 * behaviour when a bad context-classloader is encountered. When set to
	 * false, a LogConfigurationException is thrown if LogFactoryImpl is loaded
	 * via a child classloader of the TCCL (this should never happen in sane
	 * systems).
	 * 
	...
	/**
	 * The name (
	 * <code>org.apache.commons.logging.Log.allowFlawedDiscovery</code>) of the
	 * system property which can be set true/false to determine system behaviour
	 * when a bad logging adapter class is encountered during logging discovery.
	 * When set to false, an exception will be thrown and the app will fail to
	 * start. When set to true, discovery will continue (though the user might
	 * end up with a different logging implementation than they expected).
	 * 
	...
	/**
	 * The name (
	 * <code>org.apache.commons.logging.Log.allowFlawedHierarchy</code>) of the
	 * system property which can be set true/false to determine system behaviour
	 * when a logging adapter class is encountered which has bound to the wrong
	 * Log class implementation. When set to false, an exception will be thrown
	 * and the app will fail to start. When set to true, discovery will continue
	 * (though the user might end up with a different logging implementation
	 * than they expected).
	 * 
	 ...
	 * Each class is expected to implement the Log interface, and to throw
	 * NoClassDefFound or ExceptionInInitializerError when loaded if the
	 * underlying logging library is not available. Any other error indicates
	 * that the underlying logging library is available but broken/unusable for
	 * some reason.
	 */
	 ...
	 * thread-context classloader, or via the classloader that loaded this
	...
	/**
	 * The {@link org.apache.commons.logging.Log} instances that have already
	 * been created, keyed by logger name.
	 */
	@SuppressWarnings("rawtypes")
	 ...
	 * {@link org.apache.commons.logging.Log} implementation class that will be
	 * used to create new instances. This value is initialized by
	 * <code>getLogConstructor()</code>, and then returned repeatedly.
	 */
	@SuppressWarnings("rawtypes")
	 ...
	 * factory. This is useful in environments like servlet containers, which
	 ...
	 * per-component level instead of globally as system properties would do. To
	 * use this method instead of a system property, call
	 * 
	 * <pre>
	 ...
	 * <p>
	 * Note that this method does not detect whether internal logging is enabled
	 * or not, nor where to output stuff if it is; that is all handled by the
	 * parent LogFactory class. This method just computes its own unique prefix
	 * for log messages.
	 */
	...
	/**
	 * Output a diagnostic message to a user-specified destination (if the user
	 * has enabled diagnostic logging).
	 * 
	 * @param msg
	 *            diagnostic message
	 * @since 1.1
	 ...
	 * 
	 * @deprecated Never invoked by this class; subclasses should not assume it
	 *             will be.
	 */
	 ...
	 * 
	 * <p>
	 * <strong>IMPLEMENTATION NOTE</strong> - Race conditions caused by calling
	 * this method from more than one thread are ignored, because the same
	...
	/**
	 * <p>
	 * Return <code>true</code> if <em>JDK 1.4 or later</em> logging is
	 * available. Also checks that the <code>Throwable</code> class supports
	 ...
	 *             if the current java security policy doesn't allow this class
	 *             to access the context classloader.
	 */
	...
	/**
	 * Utility method to check whether a particular logging library is present
	 * and available for use. Note that this does <i>not</i> affect the future
	...
	private boolean isLogLibraryAvailable(String name, String classname) {
		...
		try {
			Log log = createLogFromClass(classname, this.getClass().getName(), // dummy
																				// category
					false);
			...
			if (isDiagnosticsEnabled()) {
				logDiagnostic("Logging system '" + name
	...
	/**
	 * Attempt to find an attribute (see method setAttribute) or a system
	 * property with the provided name and return its value.
	 * <p>
	...
	private String getConfigurationValue(String property) {
		...
		try {
			// warning: minor security hole here, in that we potentially read a
			// system
			// property that the caller cannot, then output it in readable form
			// as a
	...
	/**
	 * Initialize a number of variables that control the behaviour of this class
	 * and that can be tweaked by the user. This is done when the first logger
	 * is created, not in the constructor of this class, because we need to give
	 * the user a chance to call method setAttribute in order to configure this
	 * object.
	 */
	private void initConfiguration() {
		allowFlawedContext = getBooleanConfiguration(
				ALLOW_FLAWED_CONTEXT_PROPERTY, true);
		allowFlawedDiscovery = getBooleanConfiguration(
				ALLOW_FLAWED_DISCOVERY_PROPERTY, true);
		allowFlawedHierarchy = getBooleanConfiguration(
				ALLOW_FLAWED_HIERARCHY_PROPERTY, true);
	}

	/**
	 * Attempts to create a Log instance for the given category name. Follows
	 * the discovery process described in the class javadoc.
	 * 
	 * @param logCategory
	 *            the name of the log category
	 * 
	 * @throws LogConfigurationException
	 *             if an error in discovery occurs, or if no adapter at all can
	 *             be instantiated
	 */
	private Log discoverLogImplementation(String logCategory)
		...
		if (specifiedLogClassName != null) {
			...
			result = createLogFromClass(specifiedLogClassName, logCategory,
					true);
			if (result == null) {
				...
				if (specifiedLogClassName != null) {
					informUponSimilarName(messageBuffer, specifiedLogClassName,
							LOGGING_IMPL_LOG4J_LOGGER);
					informUponSimilarName(messageBuffer, specifiedLogClassName,
	 ...
	 * 
	 * @param messageBuffer
	 *            <code>StringBuffer</code> the message should be appended to,
	 *            not null
	 * @param name
	...
	private Log createLogFromClass(String logAdapterClassName,
			String logCategory, boolean affectState)
			throws LogConfigurationException {
		...
		for (;;) {
			...
			logDiagnostic("Trying to load '" + logAdapterClassName
			...
			try {
				if (isDiagnosticsEnabled()) {
					...
					if (url == null) {
						logDiagnostic("Class '" + logAdapterClassName + "' ["
				...
				try {
					...
					String msg = ""
							+ originalClassNotFoundException.getMessage();
		...
		if ((logAdapter != null) && affectState) {
			...
			try {
				...
				this.logMethod = null;
				logDiagnostic("[INFO] '" + logAdapterClassName
						+ "' from classloader " + objectId(currentCL)
						+ " does not declare optional method "
						+ "setLogFactory(LogFactory)");
			}

			logDiagnostic("Log adapter '" + logAdapterClassName
					+ "' from classloader "
	 ...
	 * classloader) is when an app has created custom classloaders but failed to
	 * correctly set the context classloader. This is a bug in the calling
	 * application; however we provide the option for JCL to simply generate a
	 * warning rather than fail outright.
	 * 
	 */
	...
	/**
	 * Given two related classloaders, return the one which is a child of the
	 * other.
	 * <p>
	 * 
	 * @param c1
	 ...
	 * then throws a <code>LogConfigurationException</code> that wraps the
	...
	private void handleFlawedDiscovery(String logAdapterClassName,
			ClassLoader classLoader, Throwable discoveryFlaw) {
	 ...
	 * situation is considered recoverable) or throw a
	 * LogConfigurationException.
	 * <p>
	 * There are two possible reasons why we successfully loaded the specified
	 * log adapter class then failed to cast it to a Log object:
	 * <ol>
	 * <li>the specific class just doesn't implement the Log interface (user
	 * screwed up), or
		...
		if (implementsLog) {
			...
			if (isDiagnosticsEnabled()) {
				try {
					...
					logDiagnostic("Class '" + badClass.getName()
					...
					logDiagnostic("Error while trying to output diagnostics about"
							+ " bad class '" + badClass + "'");
				}
			}
