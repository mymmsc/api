...
** Copyright (C) 2000-2009 WangFeng
...
/*lint -save -e767 */
#define VERSION     "2.7.2"         /* the current version number */
...
API_DECLARE_NONSTD(void) mwInit(void)
{
    ...
    if(!mwInfoWritten) {
        ...
        mwWrite(
            "\n============="
            " MEMWATCH 2.7.1 Copyright (C) 1992-1999 Johan Lindh "
            "=============\n"
            "============="
            " MEMWATCH " VERSION " Copyright (C) 2000-2009 Wang Feng "
            "=============\n");
        mwWrite("\nStarted at %s\n", ctime(&tid));
...
API_DECLARE_NONSTD(void) mwAbort(void)
{
    ...
    while(mwHead != NULL && errors < 3) {
        ...
        if(!(mwHead->flag & MW_NML)) {
            ...
            mwVerbose("%s(%d) : unfreed<%ld>, %ld bytes at %p \n",
                mwHead->file, mwHead->line, mwHead->count, (long)mwHead->size, data + mwOverflowZoneSize);
            if(mwCheckOF(data)) {
                mwWrite("[underflowed] ");
                FLUSH();
            }
            ...
            if(mwTestMem(data, mwHead->size, MW_VAL_NML)) {
                ...
                mwVerbose("%s(%d) : wild pointer<%ld>, %ld bytes at %p \n",
                    mwHead->file, mwHead->line, mwHead->count, (long)mwHead->size, data + mwOverflowZoneSize);
                FLUSH();
            }
            mwNmlNumAlloc --;
...
API_DECLARE_NONSTD(int) mwTest(const char *file, int line, int items)
{
    mwAutoInit();
    mwTestFlags = items;
...
*/
API_DECLARE_NONSTD(void *) mwMark(void *p, const char *desc, const char *file, unsigned line)
{
    mwMarker *mrk;
    unsigned n, isnew;
    char *buf;
    ...
    if(tot >= (int)sizeof(wherebuf)) {
        wherebuf[sizeof(wherebuf)-1] = 0; oflow = 1;
    }
    ...
    if(mrk == NULL) {
        ...
        isnew = 0;
        n = api_strlen(mrk->text);
    }
    
    n += api_strlen(wherebuf);
    buf = (char*) malloc(n+3);
    if(buf == NULL) {
        if(isnew) free(mrk);
        mwWrite("mark: %s(%d), no mark for %p:'%s', out of memory\n", file, line, p, desc);
...
API_DECLARE_NONSTD(void *) api9_malloc(size_t size, const char* file, int line)
{
    size_t needed;
    mwData *mw;
    char *ptr;
...
API_DECLARE_NONSTD(void *) api9_realloc(void *p, size_t size, const char* file, int line)
{
    int oldUseLimit, i;
    mwData *mw;
    char *ptr;
    ...
    if(p == NULL) return api9_malloc(size, file, line);
    if(size == 0) { api9_free(p, file, line); return NULL; }
    ...
    if(mwIsOwned(mw, file, line)) {
        ...
        mwUseLimit = 0;
        ptr = (char*) api9_malloc(size, file, line);
        if(ptr != NULL) {
            if(size < mw->size) {
                memcpy(ptr, p, size);
            } else {
...
API_DECLARE_NONSTD(char *) api9_strdup(const char* str, const char* file, int line) {
    size_t len;
    char *newstring;
    ...
    len = api_strlen(str) + 1;
    newstring = (char*) api9_malloc(len, file, line);
    if(newstring != NULL) memcpy(newstring, str, len);
    MW_MUTEX_UNLOCK();
    return newstring;
}

API_DECLARE_NONSTD(void) api9_free(void* p, const char* file, int line)
{
    int i;
    mwData* mw;
    ...
    if(p == NULL) {
        mwWrite("NULL free: <%ld> %s(%d), NULL pointer free'd\n",
            mwCounter, file, line);
        mwVerbose("%s(%d) : NULL free<%ld>, NULL pointer free\n",
            file, line, mwCounter);
        FLUSH();
        MW_MUTEX_UNLOCK();
        return;
    ...
    for(i = 0; i < MW_FREE_LIST; i++) {
        if(mwLastFree[i] == p) {
            ...
            mwWrite("double-free: <%ld> %s(%d), %p was"
                ...
                mwCounter, file, line, p,
                mwLFfile[i], mwLFline[i]);
            mwVerbose("%s(%d) : double-free<%ld>, %p was freed from %s(%d)\n",
                file, line, mwCounter, p, mwLFfile[i], mwLFline[i]);
            FLUSH();
            MW_MUTEX_UNLOCK();
            return;
        }
...
API_DECLARE_NONSTD(void *) api9_calloc(size_t a, size_t b, const char *file, int line)
{
    void *p;
    size_t size = a * b;
    p = api9_malloc(size, file, line);
    if(p == NULL) return NULL;
    memset(p, 0, size);
    return p;
}

API_DECLARE_NONSTD(void) mwFree_(void *p)
{
    MW_MUTEX_LOCK();
    TESTS(NULL,0);
    MW_MUTEX_UNLOCK();
...
API_DECLARE_NONSTD(void *) mwMalloc_(size_t size)
{
    MW_MUTEX_LOCK();
    TESTS(NULL,0);
    MW_MUTEX_UNLOCK();
...
API_DECLARE_NONSTD(void *) mwRealloc_(void *p, size_t size)
{
    MW_MUTEX_LOCK();
    TESTS(NULL,0);
    MW_MUTEX_UNLOCK();
...
API_DECLARE_NONSTD(void *) mwCalloc_(size_t a, size_t b)
{
    MW_MUTEX_LOCK();
    TESTS(NULL,0);
    MW_MUTEX_UNLOCK();
...
API_DECLARE_NONSTD(void) mwLimit(long lim)
{
    TESTS(NULL, 0);
...
API_DECLARE_NONSTD(int) mwAssert(int exp, const char *exps, const char *fn, int ln)
{
    int i;
...
API_DECLARE_NONSTD(int) mwVerify(int exp, const char *exps, const char *fn, int ln)
{
    int i;
    char buffer[MW_TRACE_BUFFER+8];
    if(exp) {
...
API_DECLARE_NONSTD(void) mwTrace(const char *format, ...) {
    int tot, oflow = 0;
    va_list mark;
...
API_DECLARE_NONSTD(unsigned) mwGrab(unsigned kb)
{
    TESTS(NULL,0);
    return mwGrab_(kb, MW_VAL_GRB, 0);
}

API_DECLARE_NONSTD(unsigned) mwDrop(unsigned kb)
{
    TESTS(NULL,0);
...
API_DECLARE_NONSTD(void) mwNoMansLand(int level)
{
    mwAutoInit();
    TESTS(NULL,0);
    switch(level) {
...
void* operator new(unsigned size) {
    mwNCur = 0;
    return api9_malloc(size, "<unknown>", 0);
}

/*
** This is the new operator that's called when a module uses api9_new.
*/
void* operator new(unsigned size, const char *file, int line) {
    mwNCur = 0;
    return api9_malloc(size, file, line);
}
