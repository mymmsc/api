...
** Copyright (C) 2000-2015 WangFeng
...
**
** 920810 JLI   [1.0.0]
** 920830 JLI   [1.1.0 double-free detection]
** 920912 JLI   [1.1.5 mwPuts, mwGrab/Drop, mwLimit]
** 921022 JLI   [1.2.0 ASSERT and VERIFY]
** 921105 JLI   [1.3.0 C++ support and TRACE]
** 921116 JLI   [1.4.0 mwSetOutFunc]
** 930215 JLI   [1.5.0 modified ASSERT/VERIFY]
** 930327 JLI   [1.5.1 better auto-init & PC-lint support]
** 930506 JLI   [1.5.5 MemWatch class, improved C++ support]
** 930507 JLI   [1.6.0 mwTest & CHECK()]
** 930809 JLI   [1.6.5 Abort/Retry/Ignore]
** 930820 JLI   [1.7.0 data dump when unfreed]
** 931016 JLI   [1.7.2 modified C++ new/delete handling]
** 931108 JLI   [1.7.7 mwSetAssertAction() & some small changes]
** 940110 JLI   [1.8.0 no-mans-land alloc/checking]
** 940328 JLI   [2.0.0 version 2.0 rewrite]
...
** 950913 JLI   [2.1.0 improved garbage handling]
** 951201 JLI   [2.1.1 improved auto-free in emergencies]
** 960125 JLI   [X.0.1 implemented auto-checking using mwAutoCheck()]
** 960514 JLI   [2.1.2 undefining of existing macros]
** 960515 JLI   [2.1.3 possibility to use default new() & delete()]
** 960516 JLI   [2.2.0 suppression of file flushing on unfreed msgs]
** 960516 JLI   [2.2.1 better support for using MEMWATCH with DLL's]
** 960710 JLI   [X.0.2 multiple logs and mwFlushNow()]
** 960801 JLI   [2.2.2 merged X.01 version with current]
** 960805 JLI   [2.3.0 mwIsXXXXAddr() to avoid unneeded GP's]
** 960805 JLI   [2.3.1 merged X.02 version with current]
** 961002 JLI   [2.3.2 support for realloc() + fixed STDERR bug]
** 961222 JLI   [2.4.0 added mwMark() & mwUnmark()]
** 970101 JLI   [2.4.1 added over/underflow checking after failed ASSERT/VERIFY]
** 970113 JLI   [2.4.2 added support for PC-Lint 7.00g]
** 970207 JLI   [2.4.3 added support for strdup()]
** 970209 JLI   [2.4.4 changed default filename to lowercase]
** 970405 JLI   [2.4.5 fixed bug related with atexit() and some C++ compilers]
** 970723 JLI   [2.4.6 added MW_ARI_NULLREAD flag]
** 970813 JLI   [2.4.7 stabilized marker handling]
** 980317 JLI   [2.4.8 ripped out C++ support; wasn't working good anyway]
** 980318 JLI   [2.5.0 improved self-repair facilities & SIGSEGV support]
** 980417 JLI [2.5.1 more checks for invalid addresses]
** 980512 JLI [2.5.2 moved MW_ARI_NULLREAD to occur before aborting]
** 990112 JLI [2.5.3 added check for empty heap to mwIsOwned]
** 990217 JLI [2.5.5 improved the emergency repairs diagnostics and NML]
** 990224 JLI [2.5.6 changed ordering of members in structures]
** 990303 JLI [2.5.7 first maybe-fixit-for-hpux test]
** 990516 JLI [2.5.8 added 'static' to the definition of mwAutoInit]
** 990517 JLI [2.5.9 fixed some high-sensitivity warnings]
** 990610 JLI [2.6.0 fixed some more high-sensitivity warnings]
** 990715 JLI [2.6.1 changed TRACE/ASSERT/VERIFY macro names]
** 991001 JLI [2.6.2 added CHECK_BUFFER() and mwTestBuffer()]
** 991007 JLI [2.6.3 first shot at a 64-bit compatible version]
** 991009 JLI [2.6.4 undef's strdup() if defined, api9_strdup made const]
** 000704 JLI [2.6.5 added some more detection for 64-bits]
** 010502 JLI [2.6.6 incorporated some user fixes]
...
** 010531 JLI [2.6.7 fix: mwMutexXXX() was declared even if MW_HAVE_MUTEX was not defined]
** 010619 JLI [2.6.8 fix: api9_realloc() could leave the mutex locked]
** 020918 JLI [2.6.9 changed to GPL, added C++ array allocation by Howard Cohen]
** 030212 JLI [2.7.0 api9_malloc() bug for very large allocations (4GB on 32bits)]
** 030520 JLI [2.7.1 added ULONG_LONG_MAX as a 64-bit detector (thanks Sami Salonen)]
...
** free both malloc()'d memory as well as api9_malloc()'d.
**
...
**         'api9_new' and 'delete' as 'api9_delete'. Do this *after* you include
...
#ifndef __API_MEMWATCH_H__
#define __API_MEMWATCH_H__
...
*/
API_DECLARE_NONSTD(void)  mwInit(void);
API_DECLARE_NONSTD(void)  mwTerm(void);
API_DECLARE_NONSTD(void)  mwAbort(void);
...
*/
API_DECLARE_NONSTD(void)        mwFlushNow(void);
API_DECLARE_NONSTD(void)        mwDoFlush(int onoff);
API_DECLARE_NONSTD(void)        mwLimit(long bytes);
API_DECLARE_NONSTD(unsigned)    mwGrab(unsigned kilobytes);
API_DECLARE_NONSTD(unsigned)    mwDrop(unsigned kilobytes);
API_DECLARE_NONSTD(void)        mwNoMansLand(int mw_nml_level);
API_DECLARE_NONSTD(void)        mwStatistics(int level);
API_DECLARE_NONSTD(void)        mwFreeBufferInfo(int onoff);
API_DECLARE_NONSTD(void)        mwAutoCheck(int onoff);
API_DECLARE_NONSTD(void)        mwCalcCheck(void);
API_DECLARE_NONSTD(void)        mwDumpCheck(void);
API_DECLARE_NONSTD(void *)      mwMark(void *p, const char *description, const char *file, unsigned line);
API_DECLARE_NONSTD(void *)      mwUnmark(void *p, const char *file, unsigned line);
...
*/
API_DECLARE_NONSTD(int)   mwIsReadAddr(const void *p, unsigned len);
API_DECLARE_NONSTD(int)   mwIsSafeAddr(void *p, unsigned len);
API_DECLARE_NONSTD(int)   mwTest(const char *file, int line, int mw_test_flags);
API_DECLARE_NONSTD(int)   mwTestBuffer(const char *file, int line, void *p);
API_DECLARE_NONSTD(int)   mwAssert(int, const char*, const char*, int);
API_DECLARE_NONSTD(int)   mwVerify(int, const char*, const char*, int);
...
*/
API_DECLARE_NONSTD(void)  mwTrace(const char* format_string, ...);
API_DECLARE_NONSTD(void)  mwPuts(const char* text);
API_DECLARE_NONSTD(void)  mwSetOutFunc(void (*func)(int));
API_DECLARE_NONSTD(void)  mwSetAriFunc(int (*func)(const char*));
API_DECLARE_NONSTD(void)  mwSetAriAction(int mw_ari_value);
API_DECLARE_NONSTD(int)   mwAriHandler(const char* cause);
API_DECLARE_NONSTD(void)  mwBreakOut(const char* cause);

/*
** Allocation/deallocation functions
**  These functions are the ones actually to perform allocations
**  when running MEMWATCH, for both C and C++ calls.
**  - api9_malloc() debugging allocator
**  - mwMalloc_() always resolves to a clean call of malloc()
**  - api9_realloc() debugging re-allocator
**  - mwRealloc_() always resolves to a clean call of realloc()
**  - api9_calloc() debugging allocator, fills with zeros
**  - mwCalloc_() always resolves to a clean call of calloc()
**  - api9_free() debugging free. Can only free memory which has
...
*/
API_DECLARE_NONSTD(void *) api9_malloc(size_t, const char*, int);
API_DECLARE_NONSTD(void *) mwMalloc_(size_t);
API_DECLARE_NONSTD(void *) api9_realloc(void *, size_t, const char*, int);
API_DECLARE_NONSTD(void *) mwRealloc_(void *, size_t);
API_DECLARE_NONSTD(void *) api9_calloc(size_t, size_t, const char*, int);
API_DECLARE_NONSTD(void *) mwCalloc_(size_t, size_t);
API_DECLARE_NONSTD(void)   api9_free(void*, const char*, int);
API_DECLARE_NONSTD(void)   mwFree_(void*);
API_DECLARE_NONSTD(char *) api9_strdup(const char *, const char*, int);
...
#define malloc(n)       api9_malloc(n,__FILE__, __LINE__)
#define strdup(p)       api9_strdup(p, __FILE__, __LINE__)
#define realloc(p, n)   api9_realloc(p, n, __FILE__, __LINE__)
#define calloc(n, m)    api9_calloc(n, m, __FILE__, __LINE__)
#define free(p)         api9_free(p, __FILE__, __LINE__)
...
#define api9_malloc(n, f, l)     malloc(n)
#define api9_strdup(p, f, l)     strdup(p)
#define api9_realloc(p, n, f, l)  realloc(p, n)
#define api9_calloc(n, m, f, l)   calloc(n, m)
#define api9_free(p)           free(p)
...
**  you must define 'new' to be 'api9_new', and 'delete' to be 'api9_delete'.
...
#define api9_new new(__FILE__, __LINE__)
#define api9_delete (mwNCur=1, mwNFile=__FILE__, mwNLine=__LINE__), delete
