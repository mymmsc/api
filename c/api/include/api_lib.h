...
/*
 * Define the structures used by the API general-purpose library.
 */
 ...
 * string (from api_strerror) corresponding to that error code.
 * - %%pp takes a void * and outputs it in hex
 * - %%pB takes a api_uint32_t * as bytes and outputs it's api_strfsize
 ...
 *
 * Additionally, api_vformatter allows for arbitrary output methods
 * using the api_vformatter_buff and flush_func.
 *
 * The api_vformatter_buff has two elements curpos and endpos.
 * curpos is where api_vformatter will write the next byte of output.
 * It proceeds writing output to curpos, and updating curpos, until
 * either the end of output is reached, or curpos == endpos (i.e. the
 * buffer is full).
 *
 * If the end of output is reached, api_vformatter returns the
 * number of bytes written.
 *
 * When the buffer is full, the flush_func is called.  The flush_func
 * can return -1 to indicate that no further output should be attempted,
 * and api_vformatter will return immediately with -1.  Otherwise
 ...
 * otherwise "unsafe".  For example, api_psprintf uses the "scratch"
 * space at the unallocated end of a block, and doesn't actually
 * complete the allocation until api_vformatter returns.  api_psprintf
 * would be completely broken if api_vformatter were to call anything
 * that used this same pool.  Similarly http_bprintf() uses the "scratch"
 * space at the end of its output buffer, and doesn't actually note
 * that the space is in use until it either has to flush the buffer
 * or until api_vformatter returns.
 * </PRE>
 */
API_DECLARE(int) api_vformatter(int (*flush_func)(api_vformatter_buff_t *b),
 ...
 * the provided buffer, API_ENAMETOOLONG will be returned.
 * Note that the bufsize paramater is passed by reference for no
 * reason; its value will never be modified by the api_password_get()
 * function.
 */
API_DECLARE(api_status_t) api_password_get(const char *prompt, char *pwbuf, 
                                           api_size_t *bufsize);
