...
dnl
dnl API_CONFIG_NICE(filename)
dnl
dnl Saves a snapshot of the configure command-line for later reuse
dnl
AC_DEFUN([API_CONFIG_NICE], [
  rm -f $1
  cat >$1<<EOF
#! /bin/sh
#
...
dnl API_MKDIR_P_CHECK(fallback-mkdir-p)
dnl checks whether mkdir -p works
AC_DEFUN([API_MKDIR_P_CHECK], [
...
dnl
dnl API_SUBDIR_CONFIG(dir [, sub-package-cmdline-args, args-to-drop])
dnl
...
dnl
AC_DEFUN([API_SUBDIR_CONFIG], [
  # save our work to this point; this allows the sub-package to use it
  AC_CACHE_SAVE

  echo "configuring package in $1 now"
  ac_popdir=`pwd`
  api_config_subdirs="$1"
  test -d $1 || $mkdir_p $1
...
changequote(, )dnl
      ...
      ac_dots=`echo $api_config_subdirs|sed -e 's%^\./%%' -e 's%[^/]$%&/%' -e 's%[^/]*/%../%g'`
changequote([, ])dnl
  ...
  *)  # Was relative path.
  ...
  ifelse($3, [], [api_configure_args=$ac_configure_args],[
  api_configure_args=
  api_sep=
  for api_configure_arg in $ac_configure_args
  do
    case "$api_configure_arg" in
      $3)
        continue ;;
    esac
    api_configure_args="$api_configure_args$api_sep'$api_configure_arg'"
    api_sep=" "
  done
  ])

  dnl autoconf doesn't add --silent to ac_configure_args; explicitly pass it
  test "x$silent" = "xyes" && api_configure_args="$api_configure_args --silent"

  dnl AC_CONFIG_SUBDIRS silences option warnings, emulate this for 2.62
  api_configure_args="--disable-option-checking $api_configure_args" 
  ...
  dnl will choke after doing repeated API_SUBDIR_CONFIG()s.  (Namely Solaris
  dnl and autoconf-2.54+)
  if eval $SHELL $ac_abs_srcdir/configure $api_configure_args --cache-file=$ac_sub_cache_file --srcdir=$ac_abs_srcdir $2
  then :
...
dnl API_PATH_RELATIVE(final_path, $orig_path, $prefix)
dnl    $final_path now contains "bar"
AC_DEFUN([API_PATH_RELATIVE], [
...
AC_DEFUN([API_HELP_STRING], [ifelse(regexp(AC_ACVERSION, 2\.1), -1, AC_HELP_STRING([$1],[$2]),[  ][$1] substr([                       ],len($1))[$2])])

dnl
dnl API_LAYOUT(configlayout, layoutname [, extravars])
dnl
AC_DEFUN([API_LAYOUT], [
...
dnl
dnl API_ENABLE_LAYOUT(default layout name [, extra vars])
dnl
AC_DEFUN([API_ENABLE_LAYOUT], [
...
API_LAYOUT($srcdir/config.layout, $LAYOUT, $2)
