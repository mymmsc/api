...
BEGIN {
    ...
    printf(" * that libtool will link all of the API functions\n")
...
/^[ \t]*API_POOL_DECLARE_ACCESSOR[^(]*[(][^)]*[)]/ {
    sub("[ \t]*API_POOL_DECLARE_ACCESSOR[^(]*[(]", "", $0)
    sub("[)].*$", "", $0)
    add_symbol("api_" $0 "_pool_get")
    next
}

/^[ \t]*API_DECLARE_INHERIT_SET[^(]*[(][^)]*[)]/ {
    sub("[ \t]*API_DECLARE_INHERIT_SET[^(]*[(]", "", $0)
    sub("[)].*$", "", $0)
    add_symbol("api_" $0 "_inherit_set")
    next
}

/^[ \t]*API_DECLARE_INHERIT_UNSET[^(]*[(][^)]*[)]/ {
    sub("[ \t]*API_DECLARE_INHERIT_UNSET[^(]*[(]", "", $0)
    sub("[)].*$", "", $0)
    add_symbol("api_" $0 "_inherit_unset")
    next
}
