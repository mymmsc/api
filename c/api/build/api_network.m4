...
dnl -----------------------------------------------------------------
dnl api_network.m4: API's autoconf macros for testing network support
dnl

dnl
dnl check for type in_addr
dnl
AC_DEFUN(API_TYPE_IN_ADDR,[
...
dnl
AC_DEFUN([API_CHECK_WORKING_GETADDRINFO], [
...
if test "$ac_cv_working_getaddrinfo" = "yes"; then
  if test "$ac_cv_func_gai_strerror" != "yes"; then
    ...
    AC_DEFINE(HAVE_GETADDRINFO, 1, [Define if getaddrinfo exists and works well enough for API])
  fi
fi
])

dnl Check whether the AI_ADDRCONFIG flag can be used with getaddrinfo
AC_DEFUN([API_CHECK_GETADDRINFO_ADDRCONFIG], [
  AC_CACHE_CHECK(for working AI_ADDRCONFIG, api_cv_gai_addrconfig, [
  AC_TRY_RUN([
#ifdef HAVE_NETDB_H
#include <netdb.h>
#endif
...
}], [api_cv_gai_addrconfig=yes], 
    [api_cv_gai_addrconfig=no],
    [api_cv_gai_addrconfig=no])])

if test $api_cv_gai_addrconfig = yes; then
...
dnl
AC_DEFUN([API_CHECK_WORKING_GETNAMEINFO], [
...
dnl
AC_DEFUN([API_CHECK_NEGATIVE_EAI], [
...
dnl the headers.
AC_DEFUN([API_CHECK_GETHOSTBYNAME_R_STYLE], [
...
dnl Note that this test is executed too early to see if we have all of
dnl the headers.
AC_DEFUN([API_CHECK_GETSERVBYNAME_R_STYLE], [
...
dnl
AC_DEFUN([API_CHECK_TCP_NODELAY_INHERITED], [
...
dnl on a TCP socket.
dnl
AC_DEFUN([API_CHECK_TCP_NODELAY_WITH_CORK], [
AC_CACHE_CHECK([whether TCP_NODELAY and TCP_CORK can both be enabled],
[api_cv_tcp_nodelay_with_cork],
...
#include <stdlib.h>
    ...
    exit(0);
]])], [api_cv_tcp_nodelay_with_cork=yes], [api_cv_tcp_nodelay_with_cork=no])])

if test "$api_cv_tcp_nodelay_with_cork" = "yes"; then
...
dnl
AC_DEFUN([API_CHECK_O_NONBLOCK_INHERITED], [
...
AC_DEFUN([API_CHECK_INET_NETWORK], [
...
AC_DEFUN([API_CHECK_SOCKADDR_STORAGE], [
AC_CACHE_CHECK(for sockaddr_storage, api_cv_define_sockaddr_storage,[
AC_TRY_COMPILE([
#ifdef HAVE_SYS_TYPES_H
#include <sys/types.h>
#endif
...
[api_cv_define_sockaddr_storage=yes],
[api_cv_define_sockaddr_storage=no])])

if test "$api_cv_define_sockaddr_storage" = "yes"; then
  have_sa_storage=1
else
  have_sa_storage=0
fi
...
AC_DEFUN([API_CHECK_SOCKADDR_IN6], [
...
AC_DEFUN([API_CHECK_SOCKADDR_UN], [
...
dnl
dnl API_H_ERRNO_COMPILE_CHECK
dnl
AC_DEFUN([API_H_ERRNO_COMPILE_CHECK], [
  if test x$1 != x; then
...
dnl
AC_DEFUN([API_CHECK_SCTP],
[
  AC_CACHE_CHECK([whether SCTP is supported], [api_cv_sctp], [
  AC_TRY_RUN([
#ifdef HAVE_SYS_TYPES_H
#include <sys/types.h>
#endif
...
int main(void) {
    ...
    if (setsockopt(s, IPPROTO_SCTP, SCTP_NODELAY, &opt, sizeof(int)) < 0)
       exit(2);
    exit(0);
}], [api_cv_sctp=yes], [api_cv_sctp=no], [api_cv_sctp=no])])

if test "$api_cv_sctp" = "yes"; then
    have_sctp=1
else
    have_sctp=0
fi
...
dnl API_CHECK_MCAST: check for multicast interfaces
AC_DEFUN([API_CHECK_MCAST], [
AC_CACHE_CHECK([for struct ip_mreq], [api_cv_struct_ipmreq], [
AC_TRY_COMPILE([
#include <sys/types.h>
#include <netinet/in.h>
], [
...
int main(void) { 
  ...
  if test "$ac_cv_ebcdic" = "yes"; then
    api_charset_ebcdic=1
  else
    api_charset_ebcdic=0
  fi
  AC_SUBST(api_charset_ebcdic)
])
