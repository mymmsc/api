...
# !ELSE 
# 
# ALL : "libhttpd - Win32 Release" "libapiutil - Win32 Release" "libapi - Win32 Release" "$(OUTDIR)\mod_charset_lite.so"
# 
# !ENDIF 
#...
...
#
# $(DS_POSTBUILD_DEP) : "libhttpd - Win32 Release" "libapiutil - Win32 Release" "libapi - Win32 Release" "$(OUTDIR)\mod_charset_lite.so"
#
...
sub fixcwd { 
    if (m|.mak$|) {
        ...
        $dstfl = new IO::File $tname, "w" || die;
        while ($src = <$srcfl>) {
            if (($src =~ m/^\t"(\.\.\\)+(api|api-util|api-iconv)\\.*"\\/) || 
                ($src =~ m/^\t{\$\(INCLUDE\)}".*"\\/)) {
                $verchg = -1;
            }
            else {
