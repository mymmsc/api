...
void api_hashtable_init(api_hashtable_t *table, api_hashtable_flags flags, double max_load_factor
        ...
        )
{
    ...
    if(table->array == NULL) {
        debug("api_hashtable_init failed to allocate memory\n");
    }
...
void api_hashtable_destroy(api_hashtable_t *table)
{
    unsigned int i;
    hash_entry *entry;
    hash_entry *tmp;
    ...
    if(table->array == NULL) {
        debug("api_hashtable_destroy got a bad table\n");
    }
...
void api_hashtable_insert(api_hashtable_t *table, void *key, size_t key_size, void *value,
        size_t value_size)
{
    hash_entry *entry = he_create(table->flags, key, key_size, value,
            value_size);

    api_hashtable_insert_he(table, entry);
}

// this was separated out of the regular api_hashtable_insert
// for ease of copying hash entries around
void api_hashtable_insert_he(api_hashtable_t *table, hash_entry *entry){
    hash_entry *tmp;
    unsigned int index;

    entry->next = NULL;
    index = api_hashtable_index(table, entry->key, entry->key_size);
    ...
    else
    {
        ...
        if(!(table->flags & HT_NO_AUTORESIZE) &&
            ...
            api_hashtable_resize(table, table->array_size * 2);
...
void* api_hashtable_get(api_hashtable_t *table, void *key, size_t key_size, size_t *value_size)
{
    unsigned int index  = api_hashtable_index(table, key, key_size);
...
void api_hashtable_remove(api_hashtable_t *table, void *key, size_t key_size)
{
    unsigned int index  = api_hashtable_index(table, key, key_size);
    hash_entry *entry   = table->array[index];
...
int api_hashtable_contains(api_hashtable_t *table, void *key, size_t key_size)
{
    unsigned int index  = api_hashtable_index(table, key, key_size);
    hash_entry *entry   = table->array[index];
    hash_entry tmp;
...
unsigned int api_hashtable_size(api_hashtable_t *table)
{
...
void** api_hashtable_keys(api_hashtable_t *table, unsigned int *key_count)
{
    ...
    if(ret == NULL) {
        debug("api_hashtable_keys failed to allocate memory\n");
    }
    *key_count = 0;
        ...
        while(tmp != NULL)
        {
            ...
            *key_count += 1;
            ...
            if(*key_count >= table->key_count) {
                debug("api_hashtable_keys: too many keys, expected %d, got %d\n",
...
void api_hashtable_clear(api_hashtable_t *table)
{
    api_hashtable_destroy(table);

    api_hashtable_init(table, table->flags, table->max_load_factor
#ifndef __WITH_MURMUR
...
unsigned int api_hashtable_index(api_hashtable_t *table, void *key, size_t key_size)
{
    uint32_t index;
...
void api_hashtable_resize(api_hashtable_t *table, unsigned int new_size)
{
    api_hashtable_t new_table;

    debug("api_hashtable_resize(old=%d, new=%d)\n",table->array_size,new_size);
    ...
    for(i = 0; i < table->array_size; i++)
    {
        ...
        {
            next = entry->next;
            api_hashtable_insert_he(&new_table, entry);
            entry = next;
        }
        table->array[i]=NULL;
    }
...
void api_hashtable_set_seed(uint32_t seed){
    global_seed = seed;
}
