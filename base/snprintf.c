...
static char *api_cvt(double arg, int ndigits, int *decpt, int *sign, 
                     int eflag, char *buf)
{
    register int r2;
    double fi, fj;
...
static char *api_ecvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (api_cvt(arg, ndigits, decpt, sign, 1, buf));
}

static char *api_fcvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (api_cvt(arg, ndigits, decpt, sign, 0, buf));
}

/*
 * api_gcvt  - Floating output conversion to
 * minimal length string
 */

static char *api_gcvt(double number, int ndigit, char *buf, boolean_e altform)
{
    int sign, decpt;
    register char *p1, *p2;
    register int i;
    ...
    p1 = api_ecvt(number, ndigit, &decpt, &sign, buf1);
    p2 = buf;
    if (sign)
        *p2++ = '-';
...
#define FIX_PRECISION(adjust, precision, s, s_len)  \
    if (adjust) {                                   \
        api_size_t p = (precision + 1 < NUM_BUF_SIZE) \
 ...
 * latter is faster.
 */
static char *conv_10(register api_int32_t num, register int is_unsigned,
                     register int *is_negative, char *buf_end,
                     register api_size_t *len)
{
    register char *p = buf_end;
    register api_uint32_t magnitude = num;
     ...
     */
    do {
        register api_uint32_t new_magnitude = magnitude / 10;
...
static char *conv_10_quad(api_int64_t num, register int is_unsigned,
                     register int *is_negative, char *buf_end,
                     register api_size_t *len)
{
    register char *p = buf_end;
    api_uint64_t magnitude = num;
     ...
     */
    if ((magnitude <= API_UINT32_MAX && is_unsigned)
        || (num <= API_INT32_MAX && num >= API_INT32_MIN && !is_unsigned))
            return(conv_10((api_int32_t)num, is_unsigned, is_negative, buf_end, len));
     ...
     */
    do {
        api_uint64_t new_magnitude = magnitude / 10;
...
static char *conv_in_addr(struct in_addr *ia, char *buf_end, api_size_t *len)
{
...
static char *conv_api_sockaddr(api_sockaddr_t *sa, char *buf_end, api_size_t *len)
{
    char *p = buf_end;
    int is_negative;
    api_size_t sub_len;
    ...
    *--p = ':';
    ...
    if (api_sockaddr_ip_getbuf(ipaddr_str, sa->addr_str_len, sa)) {
...
#if API_HAS_THREADS
static char *conv_os_thread_t(api_os_thread_t *tid, char *buf_end, api_size_t *len)
{
    union {
        api_os_thread_t tid;
        api_uint64_t u64;
    ...
    char *buf, api_size_t *len)
{
    ...
    if (format == 'f')
        p = api_fcvt(num, precision, &decimal_point, is_negative, buf1);
    else /* either e or E format */
        p = api_ecvt(num, precision + 1, &decimal_point, is_negative, buf1);
    ...
    if (format != 'f') {
        ...
        *s++ = format;                /* either e or E */
        decimal_point--;
        if (decimal_point != 0) {
            p = conv_10((api_int32_t) decimal_point, FALSE, &exponent_is_negative,
 ...
 */
static char *conv_p2(register api_uint32_t num, register int nbits,
                     char format, char *buf_end, register api_size_t *len)
{
...
static char *conv_p2_quad(api_uint64_t num, register int nbits,
                     char format, char *buf_end, register api_size_t *len)
{
    register int mask = (1 << nbits) - 1;
    register char *p = buf_end;
    ...
    if (num <= API_UINT32_MAX)
        return(conv_p2((api_uint32_t)num, nbits, format, buf_end, len));
...
#if API_HAS_THREADS
static char *conv_os_thread_t_hex(api_os_thread_t *tid, char *buf_end, api_size_t *len)
{
    union {
        api_os_thread_t tid;
        api_uint64_t u64;
 ...
 */
int api_vformatter(int (*flush_func)(api_vformatter_buff_t *),
					api_vformatter_buff_t *vbuff, const char *fmt, va_list ap)
{
    ...
    while (*fmt) {
        if (*fmt != '%') {
            ...
            /*
             * Modifier check.  In same cases, API_OFF_T_FMT can be
             * "lld" and API_INT64_T_FMT can be "ld" (that is, off_t is
             * "larger" than int64). Check that case 1st.
             * Note that if API_OFF_T_FMT is "d",
             * the first if condition is never true. If API_INT64_T_FMT
             * is "d' then the second if condition is never true.
             */
            if ((sizeof(API_OFF_T_FMT) > sizeof(API_INT64_T_FMT)) &&
                ((sizeof(API_OFF_T_FMT) == 4 &&
                 fmt[0] == API_OFF_T_FMT[0] &&
                 fmt[1] == API_OFF_T_FMT[1]) ||
                (sizeof(API_OFF_T_FMT) == 3 &&
                 fmt[0] == API_OFF_T_FMT[0]) ||
                (sizeof(API_OFF_T_FMT) > 4 &&
                 strncmp(fmt, API_OFF_T_FMT, 
                         sizeof(API_OFF_T_FMT) - 2) == 0))) {
                /* Need to account for trailing 'd' and null in sizeof() */
                var_type = IS_QUAD;
                fmt += (sizeof(API_OFF_T_FMT) - 2);
            }
            else if ((sizeof(API_INT64_T_FMT) == 4 &&
