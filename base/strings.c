...
//////////////////////////////////////////////////////////////////////////////////////////
static char *xxtemp = NULL;

//%%key%%
char * api_str2rep_OLD(const char *src, char * (*value_cb)(api_str_t *key, void *, size_t *), void *data)
{
	char *sRet = NULL;
	int i = 0, j = 0;
	...
	size_t tlen = len;
	if(xxtemp == NULL) {
		xxtemp = api_mallocz(API_BUFFER_MAX * 2);
	} else {
		//memset(xxtemp, 0x00, API_BUFFER_MAX * 2);
	}
	if(len <= 4) {
		sRet = api_mallocz(5);
		memcpy(sRet, src, len);
		...
		byte_t ch;
		//sRet = api_mallocz(API_BUFFER_MAX * 2);
		sRet = xxtemp;
		p = sRet;
