dnl -------------------------------------------------------- -*- autoconf -*-
dnl
dnl api_threads.m4: API's autoconf macros for testing thread support
dnl

dnl
dnl API_CHECK_PTHREADS_H([ ACTION-IF-FOUND [, ACTION-IF-NOT-FOUND]])
dnl
...
dnl
dnl API_CHECK_PTHREAD_GETSPECIFIC_TWO_ARGS
dnl
AC_DEFUN([API_CHECK_PTHREAD_GETSPECIFIC_TWO_ARGS], [
...
dnl
dnl API_CHECK_PTHREAD_ATTR_GETDETACHSTATE_ONE_ARG
dnl
AC_DEFUN([API_CHECK_PTHREAD_ATTR_GETDETACHSTATE_ONE_ARG], [
...
} ], [api_p_t_r=yes], [api_p_t_r=no], [api_p_t_r=no])

if test $api_p_t_r = yes; then
  $1
fi
...
if test -n "$api_cv_pthreads_cflags"; then
   pthreads_working=yes
   if test "x$api_cv_pthreads_cflags" != "xnone"; then
     API_ADDTO(CFLAGS,[$api_cv_pthreads_cflags])
   fi
fi

# The CFLAGS may or may not be sufficient to ensure that libapi
...
AC_CACHE_CHECK([for LIBS needed for pthreads], [api_cv_pthreads_lib], [
  api_ptc_libs=$LIBS
  for lib in -lpthread -lpthreads -lc_r; do
    LIBS="$api_ptc_libs $lib"
    API_PTHREADS_TRY_RUN([
      api_cv_pthreads_lib=$lib
      break
    ])
  done
  LIBS=$api_ptc_libs
...
if test -n "$api_cv_pthreads_lib"; then
   pthreads_working=yes
   API_ADDTO(LIBS,[$api_cv_pthreads_lib])
fi
...
dnl
dnl API_PTHREADS_CHECK_SAVE
dnl API_PTHREADS_CHECK_RESTORE
dnl
...
dnl
AC_DEFUN([API_PTHREADS_CHECK_SAVE], [
  api_pthsv_CFLAGS="$CFLAGS"
  api_pthsv_LIBS="$LIBS"
])dnl

AC_DEFUN([API_PTHREADS_CHECK_RESTORE], [
  CFLAGS="$api_pthsv_CFLAGS"
  LIBS="$api_pthsv_LIBS"
])dnl

dnl
dnl API_CHECK_SIGWAIT_ONE_ARG
dnl
AC_DEFUN([API_CHECK_SIGWAIT_ONE_ARG], [
...
AC_DEFUN([API_CHECK_PTHREAD_RECURSIVE_MUTEX], [
  AC_CACHE_CHECK([for recursive mutex support], [api_cv_mutex_recursive],
...
}], [api_cv_mutex_recursive=yes], [api_cv_mutex_recursive=no], 
[api_cv_mutex_recursive=no])])

if test "$api_cv_mutex_recursive" = "yes"; then
...
AC_DEFUN([API_CHECK_PTHREAD_ROBUST_SHARED_MUTEX], [
AC_CACHE_CHECK([for robust cross-process mutex support], 
[api_cv_mutex_robust_shared],
[AC_TRY_RUN([
#include <sys/types.h>
#include <pthread.h>
#include <stdlib.h>
...
int main(int argc, char **argv)
{
    ...
    exit(0);
}], [api_cv_mutex_robust_shared=yes], [api_cv_mutex_robust_shared=no])])

if test "$api_cv_mutex_robust_shared" = "yes"; then
